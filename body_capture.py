...
def configure_mediapipe():
    ...
    local_models_dir = os.path.join(current_dir, 'modules')
    
    if not os.path.exists(local_models_dir):
        raise FileNotFoundError(f"Models directory not found: {local_models_dir}")
    ...
    mp_models_dir = os.path.join(mp_path, 'modules')
    os.makedirs(mp_models_dir, exist_ok=True)
    
    # Copy models
    for model in required_models:
        src = os.path.join(local_models_dir, model)
        dst = os.path.join(mp_models_dir, model)
        if not os.path.exists(dst):
            import shutil
            shutil.copy2(src, dst)
    
    # Set up pose landmark directory
    pose_landmark_dir = os.path.join(mp_models_dir, 'pose_landmark')
    os.makedirs(pose_landmark_dir, exist_ok=True)
    
    # Copy pose model
    pose_model_src = os.path.join(local_models_dir, 'pose_landmark_lite.tflite')
    pose_model_dst = os.path.join(pose_landmark_dir, 'pose_landmark_lite.tflite')
    if not os.path.exists(pose_model_dst):
        import shutil
        shutil.copy2(pose_model_src, pose_model_dst)
